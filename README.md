...
* The script also installs modules for overlayfs in zram and tmpfs. Change `cryptovl` kernel option to `rootovl` (tmpfs) or `rootzram` (zram) in `/etc/grub.d/40_custom`, if you want to run overlay mode in RAM instead of an encrypted container.
